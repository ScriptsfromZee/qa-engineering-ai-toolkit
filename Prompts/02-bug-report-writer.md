...
- Application Name & Version: [e.g. TestApp v2.3.1 / Build #442]
...
- If reproduction steps cannot be fully determined from the notes, create the minimum defensible reproduction path and flag any assumptions or missing steps in the "MISSING INFORMATION FLAGS" section.
- Mark unknown values as "Unknown" rather than guessing.
- Distinguish observed behaviour from suspected causes.
...
- "Make it shorter — Slack-ready summary only". Keep the Title, Severity, Steps to Reproduce (max 3 steps), Actual Result, Workaround, drop: Full environment block, Evidence list, Additional Notes and keep a Max length of 150 words
